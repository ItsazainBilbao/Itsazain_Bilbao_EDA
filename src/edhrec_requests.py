...
def read_deck_to_dataframe(file_path):
    ...
    deck_name = lines[0].split(' ', 1)[1].strip()  # Nombre de la primera carta
...
def load_decks_from_folder(folder_path):
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".txt"):  # Solo procesamos archivos .txt
            ...
            # Leer el archivo y procesarlo en un DataFrame
            with open(file_path, 'r') as file:
                lines = file.readlines()

            # Obtener el nombre del mazo de la primera carta
            deck_name = lines[0].split(' ', 1)[1].strip()

            # Extraer los nombres de las cartas
            card_names = [line.split(' ', 1)[1].strip() for line in lines]
...
def load_decks_from_folder2(folder_path):
    all_decks = []  # Lista para almacenar los DataFrames de cada mazo
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".txt"):  # Solo procesamos archivos .txt
            ...
            # Extraer los nombres de las cartas (evitando los comandantes)
            card_names = [line.split(' ', 1)[1] for line in card_lines if ' ' in line]

            # Crear DataFrame solo con las cartas que NO sean comandantes
            df = pd.DataFrame({
                'Deck Name': [deck_name] * len(card_names),
                'Card Name': card_names
            })
    ...
    if all_decks:
        final_df = pd.concat(all_decks, ignore_index=True)
        #final_df.to_csv("all_decks.csv", index=False)  # Guardar en CSV para revisión
        return final_df
    else:
        print("No se encontraron archivos de mazos válidos en la carpeta.")
        return None
...
# Creamos df_final como una copia de df5 para no modificar el original
    df_final = df_Mazos.copy()    

    #Antes de nada vamos a eliminar las tierras básicas
    df_final = expolio(df_final)

    # Añadimos columnas vacías para "set" y "released_at"
    ...
    #Gestionamos aquí las fechas
    #df_final = gestiona_el_Release(df_final)
